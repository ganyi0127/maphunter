...
public class NWHUserUpdateParam: NSObject {
    ...
    public var heightCM: UInt?
    public var weightG: UInt?
    public var genderBoy: Bool?
...
public class NWHUser: NSObject {
    ...
    public func update(withParam param: NWHUserUpdateParam, closure: @escaping (_ resultCode: Int, _ message: String, _ data: Any?) -> ()){
        ...
        if let height = param.heightCM {
            dict["height"] = "\(height)"
        }
        if let weight = param.weightG {
            dict["weight"] = "\(weight)"
        }
        if let gender = param.genderBoy {
            dict["gender"] = gender ? "M" : "F"
        }
